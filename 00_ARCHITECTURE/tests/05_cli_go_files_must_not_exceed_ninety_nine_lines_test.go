...
func TestCLIGoFilesMustNotExceedNinetyNineLines(t *testing.T) {
	repositoryRootPath := utils.FindRepositoryRootPath(t)
	goFiles := utils.ListGoFilesInDirectory(t, repositoryRootPath, "02_CLI")
	tooLong := []string{}
	...
	slices.Sort(tooLong)
	if len(tooLong) > 0 {
		t.Fatalf("cli go files must stay at 99 lines max: %s", strings.Join(tooLong, ", "))
	}
}
